<?php
	...
	function signup()
	{
		...
		if(isset($event_json['fb_id']) && isset($event_json['first_name']) && isset($event_json['last_name']))
		{
			...
			{
			    $qrry_1="insert into users(fb_id,first_name,last_name,birthday,image1,profile_type,gender)values(";
    			$qrry_1.="'".$fb_id."',";
    			$qrry_1.="'".$first_name."',";
    			$qrry_1.="'".$last_name."',";
    			$qrry_1.="'".$birthday."',";
...
//         fwrite($handle, $data);
		...
		if(isset($event_json['fb_id']) && isset($event_json['lat_long']) )
		{
			...
			$gender=strip_tags(strtolower($event_json['gender']));
			...
			//
			$qrry_1="update users SET lat_long ='".$lat_long."' , users.lat='".$mylocation[0]."' , users.long='".$mylocation[1]."' , version='".$version."' , device='".$device."'   WHERE fb_id ='".$fb_id."' ";
			if(mysqli_query($conn,$qrry_1))
			{
			    //6371  for km query
			    $mylocation=explode(",",$lat_long);
			    ...
			    if($gender=='all')
			    {
			        $query=mysqli_query($conn,"SELECT *, ( 3959 * acos( cos( radians($mylocation[0]) ) * cos( radians( lat ) ) * cos( radians( users.long ) - radians($mylocation[1]) ) + sin( radians($mylocation[0]) ) * sin( radians( lat ) ) ) ) AS distance FROM users WHERE age BETWEEN 18 AND $age_range and fb_id !='".$fb_id."' HAVING distance < $distance ORDER BY distance LIMIT 40");
			    }
			    else
			    {
			        $query=mysqli_query($conn,"SELECT *, ( 3959 * acos( cos( radians($mylocation[0]) ) * cos( radians( lat ) ) * cos( radians( users.long ) - radians($mylocation[1]) ) + sin( radians($mylocation[0]) ) * sin( radians( lat ) ) ) ) AS distance FROM users WHERE gender = '$gender' AND age BETWEEN 18 AND $age_range and fb_id !='".$fb_id."' HAVING distance < $distance ORDER BY distance LIMIT 40");
			    }
